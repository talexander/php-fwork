...
class Validator implements \ArrayAccess {
	...
	/**
	 * Throws an exception because Validator is read-only.
	...
	public function offsetSet($offset, $value) {
		throw new Exception\Validation('Validation objects are read-only.');
	}
	...
	public function rules($field, array $rules) {
		foreach ($rules as $rule) {
			$this->rule($field, $rule[0], Util\Arr::get($rule, 1));
		}
	...
	public function check() {
        ...
        $expected = Util\Arr::merge(array_keys($original), array_keys($this->_labels));
        ...
        foreach ($expected as $field) {
            ...
            $data[$field] = Util\Arr::get($this, $field);
        ...
        foreach ($rules as $field => $set) {
            ...
            foreach ($set as $array) {
                ...
                if ( ! in_array($rule, $this->_empty_rules) AND ! Processor::notEmpty($value)) {
                    continue;
                }
	...
	public function errors($file = NULL, $translate = TRUE) {
		...
		foreach ($this->_errors as $field => $set) {
			...
			$values = array(
				':field' => $label,
				':value' => Util\Arr::get($this, $field),
			);

			if (is_array($values[':value'])) {
				// All values must be strings
				$values[':value'] = implode(', ', Util\Arr::flatten($values[':value']));
			}
